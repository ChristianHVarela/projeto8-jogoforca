...
function Jogo(props){
    const {errors, gameStart, gameStarted, palavraArray, letrasEscolhidas, acertou, errou} = props;

    return (
        <Container>
            <ImagemForca src={errors === 0 ? forca0 : errors === 1 
                ? forca1 : errors === 2 ? forca2 : errors === 3 
                ? forca3 : errors === 4 ? forca4 : errors === 5 ? 
                forca5 : forca6} />
            <ContainerDireita>
                <ContainerButton>
                    <ButtonEscolherPalavra onClick={gameStart} disabled={gameStarted}>Escolher Palavra</ButtonEscolherPalavra>
                </ContainerButton>
                <ContainerLetras>
